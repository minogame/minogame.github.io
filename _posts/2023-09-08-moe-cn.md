...
3. Gating函数中首先有一个projection-softmax结构，用以计算每一个token对于每一个expert的倾向分数[11]，比较类似于一个classifier去区分某个token要对应哪个4. expert；
5. 随后，Gating函数通过一系列吊诡的采样机制，确定每一个token最终所选择的TopK专家路径；
6. 在以上采样机制中，存在诸如Capacity/Random Token Selection等机制，保证token的对专家的分配均衡；
7. 将每一个token，dispatch到对应的专家的存贮GPU上运算，注意这里的token是temporal-independent的（类似于对于某一段输入所以提取的特征向量，后述），随后再度combine成原始的sequence；
8. 其他一切照旧，该norm的norm，该attention的attention，该residual的residual，该假装看论文摸鱼的假装看论文摸鱼。
